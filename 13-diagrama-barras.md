...
Empezamos revisando el diagrama de barras que hicimos anteriormente usando elementos `div`. Luego, vamos a adaptar ese código para dibujar las barras esta vez con SVG, lo cual nos da más flexibilidad en cuanto a la presentación visual. Por último, vamos a añadir etiquetes, de tal manera que podamos ver el valor de los datos claramente.

###El Diagrama Anterior
[Acá está lo que hicimos la última vez, con datos nuevos](http://alignedleft.com/content/03-tutorials/01-d3/130-making-a-bar-chart/1.html)
...
Puede ser difícil imaginárselo, pero podemos definitivamente mejorar este diagrama hecho con `divs`.
...
(Por supuesto, se deb nombrar `w`y `h`diferente, como `svgWidth` y `svgHeight`. Utilice lo que tenga más sentido para usted. JavaScript tiene una fijación cultural con la eficiencia, entonces es común encontrar variables de un solo caracter, código escrito sin espacios y otro tipo de sintaxis que es difícil de leer peor que es eficiente desde el punto de vista de programación.)

Luego, le decimos a D3 que cree un elemento SVG vacío y que lo añada al DOM:

    //Creata un elemento SVG 
    var svg = d3.select("body")
            .append("svg")
            .attr("width", w)
            .attr("height", h);
            
Esto inserta un nuevo elemento `<svg>` inmediatamente antes de la etiqueta de cierre `</body>`, y le asigna el ancho de y la altura de 500 por 100 pixeles. Esta cláusula también asigna el resultado a la nueva variable llamada `svg`, de tal manera que se pueda referenciar fácilmentesin tener que volver a seleccionarla en el futuro usando algo así como `d3.select("svg")`.
...
Este códig selecciona todos los `rect` dentro de `svg`. Por supuesto, aún no existen, entonces el programa devuelve una selección vacía. (Sí es raro, pero quédese y verá. Con D3 siempre es necesario seleccionar primero aquello sobre lo que va a aplicar una acción, aún cuando esa selección esté vacía por el momento.)
...
Para cada una de esas posiciones, la función `append("rect")` inserta la etiqueta `rect` en el DOM. Como aprendimos en la introducción a SVG, cada `rect`debe tener valores de `x,y, width y height` Se utiliza `attr()` para añadir estos atribuots a cada uno de los rectángulos que se acaban de crear.
...
Arreglemos primero el tema de la sobreposición. En vez de `x` con cero, se le asignará un valor dinámico que corresponde a `i`, o la posición del valor dentro del conjunto de datos. Entonces, la primera barra tendrá cero, pero las siguientes se ubicarán en el `21`, luego el `42`y así en adelante.
...
Como todo en programación, existen mil maneras de lograr ese objetivo. Acá se va a mostrar uno sencillo. Primero, se cambia la línea donde se definió la posición `x`de cada barra.
...
Ahora se debe definir el ancho de las barras para que sean proporiconales, de tal manera que se vuelvan más angostas cuando se añaden más datos o se agranden cuando hayan menos valores. Voy a añadir una nueva variable cerca de donde se definen el ancho y la altura del SVG.
...
Por último, codificamos los daos de tal forma que correspondan a la *altura* de cada barra. Pensaría uno que es tan fácil como referenciar el valor del dato `d` para asignarle la altura (`height`)  a cada barra:
...
Acá está el [diagrama de barras verde azulado](http://alignedleft.com/content/03-tutorials/01-d3/130-making-a-bar-chart/7.html). Usualmente se usa el color de la figura para mostrar algún otro atributo del dato. Esto quiere decir que se puede codificar un dato con el uso de colores. (En el caso de nuestro diagrama de barras, esto permite *codificación doble*, con lo cual un solo dato tiene dos tipos de códigos visuiales: altura y color.)
...
Acá está [este código]. No es particularmente útil desde el punto de vista visual, para ya se empieza a tener la idea de cómo se puede traducir un dato a un color. Acá, `d` se multiplica por 10, y luego se usa el valor azul en la definición del color con la función `rgb()`. Con esto, a medida que valor de `d` es mayor (más alta la barra), la barra toma un color más azul. A medida que es menor el valor, la barra tendrá menos azul (se verá más cerca del color negro).
...
Si recuerda, en la introducción a SVG se vió cómo se puede añadir elementos de texto (`texto`) a un un elemento de SVG. Se empieza con:
...
Se le hace conocido? De igual manera como se hizo con `rect`,  acá se hace con `text`. Primero, se selecciona lo que se necesita, se traen los datos, si crean nuevos elementos (que inicialmente son solamente espacios vacíos), y finalmente se añaden nuevos elementos `text` al DOM.

Vamos a añadir al código para inlcuir el valor del dato dentro de cada elemento `text`, por medio del método `text()`. 

    .text(function(d) {
        return d;
    })

y luego se va a ampliar esto aún más, incluyendo los valores `x`y `y` para ubicar el texto. La forma más fácil consiste en copiar y pegar el mismo código de x/y que se uso antes para las barras.
...
Ahora, cambiemos la manera como se calcula la posición `x` asignándole el borde izquerdo *más* la mitad del ancho de la barra:  
