...
Tome nota de que ya existe un elemento `svg` (qué bien!) y que ocupa 500 pixeles horizontales y 50 pixeles verticales. No es que se muestre mucho por el momento (qué mal!).

También tome nota de que el navegador asumió que la medida por defecto es el *pixel*. Se especificó las dimensiones de `500` y `50` y no `500px`y `50px`. Se habría podido especificar `px` explícitamente, o cualquier otro tipo de unidad de medida, incluyendo `em`, `pt`, `in`, `cm`, y `mm`.
